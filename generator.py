...
def generate_Bstar_failure(n, m):
    """
    ...
    beta_1 = 3 * ((n * np.log(n)) ** 0.25) / np.sqrt(m)

    I = np.random.choice(n, m+10, replace=False)  # indices 0 to m-1
...
def symmetric_gaussian_noise_heter(n, mean=0, sdmax=sqrt(2), sdmin=sqrt(0.8)):
    Sigma = np.zeros((n, n))
