...
def glasso(Y, m, rho=1, max_iter=30, verbose=False):
    n = Y.shape[0]
    c0 = (np.partition(np.linalg.norm(Y, axis=1), -(m-1))[-(m-1)] - sqrt(n))/((n**(0.25))*(log(n)**(0.25)))
    for i in range(max_iter):
        ...
        if len(Ihat) < m:
            ...
            break 
    if verbose:
        u.heatmap(V + V.T, title="Group Lasso")
